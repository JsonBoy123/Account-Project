   ...
   public function store(Request $request){
    //dd($request->all());
       ...
       // 'other'=>'nullable',
        ...
        'five_percrnt_sd_amount'=>'required',
        'gst_amount'=>'required|numeric',
        'mobilize_amount'=>'required|numeric',
        'retent_amount'=>'required|numeric',
        'other_deduct_amount'=>'required|numeric',
        'total_deduct_amount'=>'required|numeric',
        'gst_hold'=>'required|numeric',
        'balance_to_be_billed'=>'required|numeric',
        'amount_to_be_received'=>'required|numeric',
        'comp_id'=>'required|numeric',
        'tds_on_gst_2per'=>'required|numeric',
        'lab_cess_1per'=>'required|numeric',
        'hold_for_royalty'=>'required|numeric',
        'deb_late_submission_on_car_151day'=>'required|numeric',
...
// ->increment('count', 1, ['last_count_increased_at' => Carbon::now()]);
   ...
   public function saledetails($id){
    $edit = sales::with('company','job')->where('id',$id)->first();
   ...
   public function generatebill($id){
        ...
        $gene = sales::with('company','job')->where('id',$id)->first();
        $ckamt = sales::find($id)->checkamount;
        $pdf = PDF::loadView('pages.billPdf', compact('gene','ckamt'));
      
  
        return $pdf->download('clientbill.pdf');
   }
