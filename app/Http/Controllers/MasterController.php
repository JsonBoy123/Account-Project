...
class MasterController extends Controller
{
    ...
    public function recitem($id){
        $user = Auth::user()->id;
        $siteuser = siteuser::where(['user_id'=> $user,'site_id'=>$id,'user_status' => 'active'])->latest()->first();
        if($siteuser){
        $acuser = EmpMast::where('user_id',$siteuser->user_id)->first()->emp_name;
        $grcode = $siteuser->grcode;
        $item = itemqtynew::with('itemname','house')->where('site_id',$id)->where('quantity','<>',0)->get();
        // return $item[0]->item_in;
        $count = count($item);
        $sum = itemqtynew::where('site_id',$id)->where('item_in','<>',0)->sum('item_in');
          $site = job_master::find($id)->job_describe;
          $active = siteuser::where('user_status','active')->pluck('user_id')->unique();
          $canuser = EmpMast::whereNotIn('user_id',$active)->where(['dept_id' =>'3','deleted_at'=> Null])->get();

        return view("Master.recitem",compact('item','site','grcode','user','sum','count','canuser','acuser'));
            } else{
          $site = job_master::find($id)->job_describe;
        return view("Master.unauth",compact('site'));
            }
    }

    public function sitein(Request $request){

        $count = count($request->item);
        for($i=0;$i<$count;$i++){

            itemqtynew::where(['site_id' => $request->site_id,'item_number' => $request->item[$i],'wareh_id' => $request->wareh_id[$i]])->update(['item_in' => 1]);
        }
  


        return redirect()->back()->with('mgs','Item Are Taken In Site');

    }
      public function siteagain($site,$wh,$itno){
           // return $wh."--".$itno;
              itemqtynew::where(['site_id' => $site,'item_number' => $itno,'wareh_id' => $wh])->update(['item_in' => 1]);
              return redirect()->back()->with('mgs','Item Are Taken In Site');
        }

    public function itemws($id){

        $item = itemqtynew::with('itemname')->where('site_id',$id)->where('quantity','<>',0)->get();
        $site = job_master::find($id)->job_describe;
        return view("Master.itemonsite",compact('item','site'));
