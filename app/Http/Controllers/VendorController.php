...
class VendorController extends Controller
{
    ...
    {  
    	$ven_type = VendorType::all();
        $firm_type = FirmType::all();
        $states = state::all();
        return view('vendor.create', compact('ven_type','firm_type','states'));
    }
    ...
    {  
        $data = $request->validate([
            ...
            'website' => 'nullable',
            'pan_no' => 'required|unique:acco_vendor_mast',
            'aadhar_no' => 'nullable',
            ...
            'account_no' => 'nullable',
            'sgst_number' => 'nullable',
            'max_limit' => 'nullable',
            'vendor_cat_id' => 'nullable',
            
        ]);

        if($request->gst_type =='Reg'){
            $request->validate([
                'gst_number'            => 'required',
                'sgst_number'            => 'nullable',
    ...
    public function show($id)
    {
        ...
        $vendors  = VendorMast::find($id);
        return view('vendor.details',compact('vendors'));
    }
    ...
    { 
        ...
        $firm_type = FirmType::all();
        $states = state::all();
        return view('vendor.edit',compact('vendors','ven_type','firm_type','states'));
    }
    ...
    public function update(Request $request, $id)
    {
       $update = $request->validate([
            ...
            VendorMast::where('id',$id)->update($update);
    ...
    public function vendorexcel(Request $request){
      ...
      //return $error_name."".$status;
            ...
            if($status){
              if($item['vendor_type']) {        
                $vtype = VendorType::where('name',$item['vendor_type'])->first();
                if($vtype){
                   $status = true;
                }else{
                 $status = true;
            ...
            if($status){
              if($item['firm_type']) {        
                $ftype = FirmType::where('name',$item['firm_type'])->first();
                if($ftype){
                   $status = true;
                }else{
                 $status = true;
